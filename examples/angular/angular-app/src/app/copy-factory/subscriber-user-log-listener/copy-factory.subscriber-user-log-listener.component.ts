...
@Component({
  selector: 'copy-factory-subscriber-user-log-listener',
  templateUrl: './copy-factory.subscriber-user-log-listener.component.html',
})

export class CopyFactorySubscriberUserLogListenerComponent implements OnInit, OnDestroy {
  ...
  fetchData = async () => {
    try {
      ...
      this.disconnect = await this.copyFactoryService.subscriberUserLogListener(this.subscriberAccountId!);
    } catch(err) {
      console.log(err);
      throw err;
    }

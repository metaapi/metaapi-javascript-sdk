...
interface IMetaStatsGetTradesProps {
  accountId?: string
  domain?: string
  token?: string
}
export function MetaStatsGetTrades({
  accountId: defaultAccountId, token: defaultToken , domain: defaultDomain
}: IMetaStatsGetTradesProps) {
  /* UI control */
  ...
  useEffect(() => {
    ...
    const fetchAccountTrades = async (startTime: string, endTime: string) => {
      ...
      try {
        ...
        const trades = await metaStats.getAccountTrades(accountId, startTime, endTime);
        log(trades.slice(-5));

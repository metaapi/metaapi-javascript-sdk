...
watch(isConnecting, () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const equityBalanceListener = new ExampleEquityBalanceListener(accountId.value);
      const listenerId = await riskManagementApi.addEquityBalanceListener(equityBalanceListener, accountId.value);
...
<template>
  <Section>
    <h1>Risk management. Equity balance</h1>

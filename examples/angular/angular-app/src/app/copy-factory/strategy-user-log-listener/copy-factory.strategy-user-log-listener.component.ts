...
@Component({
  selector: 'copy-factory-strategy-user-log-listener',
  templateUrl: './copy-factory.strategy-user-log-listener.component.html',
})

export class CopyFactoryUserLogListenerComponent implements OnInit, OnDestroy {
  ...
  fetchData = async () => {
    try {
      ...
      this.disconnect = await this.copyFactoryService.strategyUserLogListener(this.strategyId!);
    } catch(err) {
      console.log(err);
      throw err;
    }

...
interface ICopyFactorySubscriberUserLogListenerProps {
  subscriberAccountId?: string
  domain?: string
  token?: string
}

export function CopyFactorySubscriberUserLogListener({
  subscriberAccountId: defaultSubscriberAccountId,
  domain: defaultDomain,
  token: defaultToken
}: ICopyFactorySubscriberUserLogListenerProps) {
  /* UI control */
  ...
  */
  ...
  useEffect(() => {
    ...
    const makeRequest = async () => {
      ...
      try {
        ...
        listenerIdRef.current = tradingApiRef.current.addSubscriberLogListener(listener, subscriberAccountId);
        log('Start listening subscriber logs');
      }
      catch (err){
        logErr(err);
        throw err;
    ...
    const disconnect = () => {
      if (listenerIdRef.current && tradingApiRef.current) {
        tradingApiRef.current.removeSubscriberLogListener(listenerIdRef.current);
  ...
  return (
    <Sections>
      <h1>CopyFactory. Subscriber Listener</h1>
